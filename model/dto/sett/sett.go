...
// Get 获取总设置
func (s *Sett) Get(res *SetRes) error {
	var conbanner []ConBannerRes
	var contag []ConTagRes
	var myapp []MyAppRes
	var bag []BagRes
...
type SetResAdmin struct {
	...
	Bag           int         `gorm:"default:1" json:"bag"`        // 1 默认显示  2 不显示  钱包
	WebLink       int         `gorm:"default:1" json:"web_link"`   // 1 默认显示  2 不显示  星聊网页版链接
	ConBannerRes  []ConBanner `json:"con_banners"`                 // 联系人banner列表
	ConTagRes     []ConTag    `json:"con_tags"`                    // 联系人标签列表
	MyAppRes      []MyApp     `json:"my_apps"`                     // 我的应用列表
	BagRes        []Bag       `json:"bags"`                        // 钱包列表
	NavFunRes     []NavFun    `json:"nav_funs"`                    // 左侧菜单功能列表
...
// GetAdmin 获取总设置
func (s *Sett) GetAdmin(res *SetResAdmin) error {
	var banner []ConBanner
	var tag []ConTag
	var myapp []MyApp
	var bag []Bag
	var navfun []NavFun
